<template>
  <view class="u-index-list">
    ...
    <!-- #endif -->
    <view class="u-index-list__letter" ref="u-index-list__letter" :style="{ top: $u.addUnit(letterInfo.top || 100) }"
          @touchstart="touchStart" @touchmove.stop.prevent="touchMove" @touchend.stop.prevent="touchEnd"
      ...
      <view class="u-index-list__letter__item" v-for="(item, index) in uIndexList" :key="index" :style="{
        ...
        <text class="u-index-list__letter__item__index" :style="{color: activeIndex === index ? '#fff' : inactiveColor}">{{ item }}</text>
      </view>
    </view>
 ...
 * @property {String}			inactiveColor	右边锚点非激活的颜色 ( 默认 '#757575' )
