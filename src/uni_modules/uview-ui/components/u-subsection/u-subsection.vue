<template>
  <view class="u-subsection" ref="u-subsection" :class="[`u-subsection--${mode}`]" :style="[$u.addStyle(customStyle), wrapperStyle]">
    <view class="u-subsection__bar" ref="u-subsection__bar" :style="[barStyle]" :class="[
    ...
    <view class="u-subsection__item" :class="[
            ...
            ]" :ref="`u-subsection__item--${index}`" :style="[itemStyle(index)]" @tap="clickHandler(index)" v-for="(item, index) in list"
          :key="index">
 ...
 * @property {String}			inactiveColor	未激活时的颜色（默认 '#212121' ）
...
export default {
      ...
      handler(n) {
        ...
        uni.$u.sleep(ref ? 0 : 100).then(() => {
          animation.transition(this.$refs["u-subsection__bar"].ref, {
            styles: {
              transform: `translateX(${n * this.itemRect.width
                }px)`,
  ...
  computed: {
    ...
    barStyle() {
      ...
      // #ifndef APP-NVUE
      style.transform = `translateX(${this.current * this.itemRect.width
        }px)`;
      // #endif
