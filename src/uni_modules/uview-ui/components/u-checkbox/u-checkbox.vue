<template>
  <view class="u-checkbox" :style="[checkboxStyle]" @tap.stop="wrapperClickHandler"
        :class="[`u-checkbox-label--${parentData.iconPlacement}`, parentData.borderBottom && parentData.placement === 'column' && 'u-border-bottom']">
    <view class="u-checkbox__icon-wrap" @tap.stop="iconClickHandler" :class="iconClasses" :style="[iconWrapStyle]">
      <slot name="icon">
        <u-icon class="u-checkbox__icon-wrap__icon" name="checkbox-mark" :size="elIconSize" :color="elIconColor" />
      </slot>
    </view>
    <text class="u-checkbox__text" @tap.stop="labelClickHandler" :style="{
...
export default {
  ...
  computed: {
    ...
    // label的颜色
    elLabelColor() {
      return this.labelColor ? this.labelColor : (this.parentData.labelColor ? this.parentData.labelColor : '#757575')
    },
    // 组件的形状
    elShape() {
